    ...
    using coord = std::pair<int, int>;
        ...
        GridWorld(int size, int x0, int y0, int xA, int yA, Actor agent) : 
            size(size), n_states(size*size), agent(agent) {
                ...
                grid = stringGrid(size, std::vector<std::string>(size, "-"));

                Q_table = doubleGrid(size*size, std::vector<double>(n_actions, 0.0));
        ...
        doubleGrid convertGreedyQtableToGrid(){
            ...
            doubleGrid Q_tableGrid(size, std::vector<double>(size, 0));
